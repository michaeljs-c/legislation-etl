...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Local ETL for legislation to Sql Server"
    )
    parser.add_argument(
        "-D", "--driver", help="SQL Server driver string", default=DRIVER
    )
    parser.add_argument(
        "-S",
        "--server",
        help="SQL Server server IP address (use host IP if using SS container)",
        default=SERVER,
    )
